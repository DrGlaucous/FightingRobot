...
#define DIGITAL_CHANNELS_CT DIGITAL_CHANNEL_CNT * 2
#endif

#ifndef ANALOG_CHANNELS_CT
#define ANALOG_CHANNELS_CT ANALOG_CHANNEL_CNT
#endif
...
#pragma pack(1) 
typedef union RFM_Packet {
    RFM_ResponsePacket response;
    RFM_ControllerPacket control;
} RFM_Packet;


//note: is a singleton class
class RFM_RadioHandler {

private:

    //only one instance at a time
    static RFM_RadioHandler* instance;
...
public:
    ...
    bool sendPacket(RFM_Packet* packet, ptype packet_type, uint16_t address, bool use_ack);

    //returns true if the packet is new and false if this method was called multiple times before the next rx
    bool checkForPackets(RFM_Packet& packet_clone, ptype& packet_type);
